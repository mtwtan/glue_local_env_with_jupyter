...
#### PRE-REQUISITES ####
# Assumes that this repository is cloned to $gitclonedir/glue_local_env_with_jupyter
# Make sure that git is installed
########################

cd $HOME
dns1="<Enter IP address>" 
dns2="<Enter IP address>"
ldapbinduser="<AD username of the ldap bind authorized to search and authenticate>"
ldapbinduserpw="<Password to the above AD user>"
gitclonedir="/home/ec2-user" ## Change this if a different folder is used for downloading the git repo
...
sudo sh -c 'echo "c.JupyterHub.authenticator_class = \"ldapauthenticator.LDAPAuthenticator\"" >> /opt/jupyterhub/etc/jupyterhub/jupyterhub_config.py'
sudo sh -c 'echo "c.LDAPAuthenticator.server_address = \"tanmatth-aws.com\"" >> /opt/jupyterhub/etc/jupyterhub/jupyterhub_config.py'
...
sudo cp $gitclonedir/glue_local_env_with_jupyter/common/kernel.json /opt/jupyterhub/share/jupyter/kernels/gluepyspark
...
sudo sh -c "cp ${gitclonedir}/glue_local_env_with_jupyter/common/nginx.conf /etc/nginx/nginx.conf"
sudo sh -c "cp ${gitclonedir}/glue_local_env_with_jupyter/jupyterhub/jupyterhub.conf /etc/nginx/conf.d/jupyterhub.conf"
